---
description: MotherDuck-inspired front-end style guardrails (Happy Hues palette 12)
...
- Root background is soft lavender `#d4d8f0`. Never ship pure white canvases; all pages start here.
...
- The eyebrow banner is always sunbeam (#eebbc3) with uppercase CTA text, spanning edge-to-edge and pinned above navigation.
- Section stacks alternate between cream (#d4d8f0), soft pink (#eebbc3), and gridded cards to keep rhythm. Use wavy SVG separators or simple 2px graphite (#121629) rules when switching backgrounds.

## Color System

Uses [Happy Hues Palette 12](https://www.happyhues.co/palettes/12).

```css
:root {
  --md-cream: #d4d8f0;        /* page shell, light sections */
  --md-sunbeam: #eebbc3;      /* eyebrow, promo strips, button, highlight */
  --md-sunbeam-dark: #d4939d; /* hover/pressed, newsletter accent */
  --md-sky: #eebbc3;          /* primary CTA fill */
  --md-sky-strong: #b8c1ec;   /* focus rings, active inputs */
  --md-soft-blue: #d4d8f0;    /* secondary sections */
  --md-cloud: #fffffe;        /* cards & quotes */
  --md-fog: #fffffe;          /* form fills */
  --md-ink: #232946;          /* primary text, headline on light */
  --md-slate: #b8c1ec;        /* secondary text, paragraph */
  --md-graphite: #121629;     /* structural strokes */
  --md-shadow: rgba(18, 22, 41, 0.12); /* hover shadows */
  --md-grid-line: #b8c1ec;    /* graph-paper overlays */
}
```

- Minimum text contrast: 5:1 on cream/soft-blue, 7:1 when text sits on sunbeam or dark backgrounds.
- Sunbeam and sky saturations deepen on hover via `--md-sunbeam-dark`; darker states must still contrast with graphite outlines.
- Use opacity (not new hues) for muted variants. `rgba(184, 193, 236, 0.4)` is the tinted highlight.
...
- Highlight cards (testimonials) add a sunbeam (#eebbc3) badge tab or duck icon anchored to the top-left with `position: absolute`.
...
- Loading uses duck GIFs or skeleton shimmer `linear-gradient(90deg, #d4d8f0 25%, #fffffe 50%, #d4d8f0 75%)`.
...
- Gradients stay subtle (sunbeam #eebbc3 → cream #d4d8f0). Avoid neon or multicolor blends.
...
- ❌ Ship white background pages—cream shell (#d4d8f0) is mandatory.

## Implementation Checklist

1. Import the tokens above into `design-tokens.css`; never hardcode ad-hoc hex values.
2. Set `body { background: var(--md-cream); font-family: var(--font-family-primary); }` (cream = #d4d8f0).
...
5. Inputs/checkboxes must reuse the fog fill and sky focus color (`#b8c1ec`).
